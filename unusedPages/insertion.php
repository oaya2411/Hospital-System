...
<form class="addNew" action='doc.php' method='post'>
    <button style="background-color:ghostwhite;border-style:none;cursor:pointer" type="submit" name="doc" id="addButton"><img style="background-color:ghostwhite;" title="Add New Doctor" height="50px" weight="50px" src="add-user.png"></button>
</form>
...
<form class="form1" method="post" action="doc.php">
    <div class="button-container" onmouseover="animateButton(this)" onmouseout="resetButton(this)">
        <button type="submit" name="sub" class="image-container">
            <img title="Display Doctors Info" id="docPic" src="doc.png">
        </button>
        <div class="TextArea">
    ...
    if(isset($_POST['sub'])){
            ...
            if(sqlsrv_has_rows($result)){
                ...
                while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)){
                    ...
                    echo "<td>" . '<form method="post" action="doc.php"><button type="submit" id="deleteButton" class="basket" name="update['.$row['doctor_id'].']"><img title="Update Data" height="50px" width="50px" src="edit.png"></button>';
